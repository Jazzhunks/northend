...
// 1. Hard Whitelist for Open Redirect Targets
...
export default function Login() {
  ...
  const [showPassword, setShowPassword] = useState(false); // 22. Password Visibility Toggle
  const [inlineError, setInlineError] = useState(""); // Captures error states to show inline popup
  
  // Refs tracking execution metadata to prevent interface spams
  const abortControllerRef = useRef(null);
  const lastAttemptRef = useRef(0);

  // Clean up any lingering network request handshakes if component unmounts mid-flight
  useEffect(() => {
    return () => abortControllerRef.current?.abort();
  }, []);

  // 1. Strict Open Redirect Whitelist Validation
  ...
  const submit = async (e) => {
    e.preventDefault();
    setInlineError(""); // Clear any previous errors on submit

    // 11. Prevent Request Spamming (Client-side Execution Cooldown)
    const now = Date.now();
    ...
    // 6. Client-Side Integrity Checks
    ...
    // 8. Abort Pending Active Requests
    ...
    // 10. Request Timeout Implementation
    ...
    try {
      ...
      toast.success("Welcome back!"); // Keeping the success toast global
      ...
      // 429 Interception & 9/24. Anti-Enumeration Fallbacks
  ...
  return (
    <>
      {/* 2. Inject Dynamic Canonical Metadata */}
      <Helmet>
        ...
        <div className="relative flex items-center justify-center p-6 lg:p-12">
          ...
          >
            ...
            <GlassPanel elevated className="p-8">
              ...
              {/* Inline Error Popup Area inside the Card container */}
              ...
              <form onSubmit={submit} className="space-y-3">
                ...
                {/* Password Input Field with Masking Toggles */}
                <div className="relative">
