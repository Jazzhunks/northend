      ...
      <div className="flex justify-between items-end flex-wrap gap-4 shrink-0">
        ...
        <div className="flex items-center gap-3">
          <a href={`${API_BASE}/erp/exports/students.xlsx`} target="_blank" rel="noreferrer" className="block">
            <button className="px-4 py-2.5 border border-white/10 rounded-xl text-xs uppercase tracking-wider font-bold text-muted-foreground hover:text-foreground hover:bg-white/5 flex items-center gap-2 transition" data-testid="export-students-btn">
          ...
          <table className="w-full text-sm table-fixed border-collapse min-w-[850px]">
            <thead className="bg-[#18181b] text-muted-foreground sticky top-0 z-20 shadow-[0_1px_0_rgba(255,255,255,0.05)]">
              <tr className="text-left backdrop-blur-md">
                <th className="w-[15%] px-5 py-3.5 text-xs font-bold uppercase tracking-wider bg-[#131316]">Student No</th>
                <th className="w-[20%] px-5 py-3.5 text-xs font-bold uppercase tracking-wider bg-[#131316]">Full Name</th>
                <th className="w-[15%] px-5 py-3.5 text-xs font-bold uppercase tracking-wider bg-[#131316]">Contact Line</th>
                <th className="w-[16%] px-5 py-3.5 text-xs font-bold uppercase tracking-wider bg-[#131316]">Allocated Branch</th>
                <th className="w-[12%] px-5 py-3.5 text-xs font-bold uppercase tracking-wider text-right bg-[#131316]">Net Payable</th>
                <th className="w-[10%] px-5 py-3.5 text-xs font-bold uppercase tracking-wider bg-[#131316]">Admission</th>
                <th className="w-[12%] bg-[#131316]"></th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/[0.04] bg-background/20">
              {items.map(s => {
                ...
                return (
                  <tr key={s.id} className="hover:bg-white/[0.02] transition-colors group" data-testid={`student-row-${s.id}`}>
...
function CreateStudentModal({ erpUser, branches, onClose, onCreated }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 z-50 grid place-items-center p-4 backdrop-blur-sm overflow-y-auto animate-fadeIn" onClick={onClose} data-testid="create-student-modal">
      <form onClick={e => e.stopPropagation()} onSubmit={submitAdmissionRecord} className="bg-background border border-white/10 rounded-2xl max-w-2xl w-full p-6 space-y-5 my-8 shadow-2xl relative">
        ...
        <div className="grid sm:grid-cols-2 gap-4 max-h-[55vh] overflow-y-auto px-1 custom-scrollbar">
          ...
          <div className="sm:col-span-2">
            ...
            <textarea value={form.address} onChange={e => setForm({...form, address: e.target.value})} rows={2} placeholder="Complete geographical verification details..." className="w-full px-3 py-2 border border-white/10 bg-background/50 rounded-xl text-sm text-foreground focus:outline-none focus:border-accent transition resize-none" data-testid="cs-address"/>
          </div>
        </div>

        {/* Dynamic State Preview Mathematical Feedback Row */}
        <div className="p-4 border border-dashed border-white/10 rounded-xl bg-white/[0.01] flex items-center justify-between shrink-0">
          <div>
          ...
          <div className="text-right">
            ...
            <div className="text-xl font-bold font-mono text-emerald-400 mt-0.5">{fmtINR(previewNetFeePayable)}</div>
          </div>
        </div>
    ...
    <div>
      ...
      <div className="relative">
        ...
        <input 
          ...
          className={`w-full pr-3 py-2 border border-white/10 bg-background/50 rounded-xl text-sm focus:outline-none focus:border-accent transition text-foreground ${Icon ? "pl-9" : "px-3"}`} 
          data-testid={testid}
        />
      </div>
    </div>
  ...
  return (
    <div>
      ...
      <select 
        ...
        required={req} 
        disabled={disabled}
        className="w-full px-3 py-2 border border-white/10 bg-background rounded-xl text-sm text-foreground focus:outline-none focus:border-accent transition disabled:opacity-50" 
        data-testid={testid}
      >
