      ...
      <div className="flex gap-3 flex-wrap shrink-0">
        <div className="relative flex-1 min-w-[250px]">
          ...
          <input 
            ...
            className="w-full pl-9 pr-4 py-2 border border-white/10 bg-background/50 rounded-xl text-sm focus:outline-none focus:border-accent/40 transition font-sans text-foreground"
          />
        </div>
        <select 
          value={branchId} 
          ...
          <table className="w-full text-sm table-auto border-collapse">
            <thead className="bg-[#18181b] text-muted-foreground sticky top-0 z-20 shadow-[0_1px_0_rgba(255,255,255,0.05)]">
              <tr className="text-left backdrop-blur-md">
                <th className="px-5 py-3.5 text-xs font-bold uppercase tracking-wider bg-[#131316]">Timestamp</th>
                <th className="px-5 py-3.5 text-xs font-bold uppercase tracking-wider bg-[#131316]">System Actor</th>
                <th className="px-5 py-3.5 text-xs font-bold uppercase tracking-wider bg-[#131316]">Role Access</th>
                <th className="px-5 py-3.5 text-xs font-bold uppercase tracking-wider bg-[#131316]">Action</th>
                <th className="px-5 py-3.5 text-xs font-bold uppercase tracking-wider bg-[#131316]">Target Entity</th>
                <th className="px-5 py-3.5 text-xs font-bold uppercase tracking-wider bg-[#131316]">Payload Matrix</th>
              </tr>
            </thead>
              ...
              {filteredItems.map(a => (
                <tr key={a.id} className="hover:bg-white/[0.02] transition-colors group">
                  ...
                  <td className="px-5 py-4 text-xs">
                    <span className="text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded bg-white/5 border border-white/5 text-muted-foreground">
...
function ActionBadge({ action }) {
  ...
  if (norm.includes("create") || norm.includes("post") || norm.includes("admit")) {
    ...
    config = "bg-rose-500/10 text-rose-400 border-rose-500/20";
  ...
  return (
    <div className="fixed inset-0 bg-black/70 z-50 grid place-items-center p-4 backdrop-blur-sm animate-fadeIn" onClick={onClose}>
      <div 
        ...
        className="bg-background border border-white/10 rounded-2xl max-w-xl w-full p-6 space-y-5 shadow-2xl overflow-hidden flex flex-col max-h-[85vh]"
      >
        ...
        <div className="space-y-2 text-xs border border-white/5 bg-white/[0.01] p-4 rounded-xl font-mono shrink-0">
        ...
        <div className="flex-1 overflow-y-auto rounded-xl border border-white/5 bg-[#09090b] p-4">
          <pre className="text-xs text-emerald-400 font-mono leading-relaxed whitespace-pre-wrap font-medium">
