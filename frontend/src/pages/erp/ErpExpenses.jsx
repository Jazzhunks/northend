  ...
  return (
    <div className="space-y-6 h-[calc(100vh-120px)] flex flex-col min-h-0 animate-fadeIn" data-testid="erp-expenses-page">
      ...
      <div className="flex justify-between items-end flex-wrap gap-4 shrink-0">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            {filteredItems.length} records parsed • Total approved settlement: <span className="text-rose-400 font-semibold font-mono">{fmtINR(total)}</span>
          </p>
        </div>
        <div className="flex items-center gap-3">
          <a href={`${API_BASE}/erp/exports/expenses.xlsx`} target="_blank" rel="noreferrer" className="block">
            <button className="px-4 py-2.5 border border-white/10 rounded-xl text-xs uppercase tracking-wider font-bold text-muted-foreground hover:text-foreground hover:bg-white/5 flex items-center gap-2 transition" data-testid="export-expenses-btn">
      ...
      <div className="flex gap-3 flex-wrap shrink-0">
        ...
        {isSuper(erpUser) && (
          <select value={branchId} onChange={e => setBranchId(e.target.value)} className="border border-white/10 rounded-xl px-4 py-2 bg-background/50 text-sm min-w-[200px] focus:outline-none text-foreground" data-testid="filter-branch">
            <option value="">All network branches</option>
            {branches.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}
          </select>
        )}
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="border border-white/10 rounded-xl px-4 py-2 bg-background/50 text-sm min-w-[200px] focus:outline-none text-foreground" data-testid="filter-status">
          ...
          <table className="w-full text-sm table-fixed border-collapse min-w-[900px]">
            <thead className="bg-[#18181b] text-muted-foreground sticky top-0 z-20 shadow-[0_1px_0_rgba(255,255,255,0.05)]">
              <tr className="text-left backdrop-blur-md">
                <th className="w-[12%] px-5 py-3.5 text-xs font-bold uppercase tracking-wider bg-[#131316]">Date</th>
                <th className="w-[15%] px-5 py-3.5 text-xs font-bold uppercase tracking-wider bg-[#131316]">Category</th>
                <th className="w-[28%] px-5 py-3.5 text-xs font-bold uppercase tracking-wider bg-[#131316]">Description</th>
                <th className="w-[15%] px-5 py-3.5 text-xs font-bold uppercase tracking-wider bg-[#131316]">Vendor / Party</th>
                <th className="w-[10%] px-5 py-3.5 text-xs font-bold uppercase tracking-wider bg-[#131316]">Recorded By</th>
                <th className="w-[10%] px-5 py-3.5 text-xs font-bold uppercase tracking-wider bg-[#131316]">Status</th>
                <th className="w-[15%] px-5 py-3.5 text-xs font-bold uppercase tracking-wider text-right bg-[#131316]">Amount</th>
                <th className="w-[8%] px-5 py-3.5 bg-[#131316]"></th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/[0.04] bg-background/20">
              {filteredItems.map(e => (
                <tr key={e.id} className="hover:bg-white/[0.02] transition-colors group" data-testid={`exp-row-${e.id}`}>
                  <td className="px-5 py-4 text-xs whitespace-nowrap text-muted-foreground font-mono">{fmtDate(e.expense_date)}</td>
                  <td className="px-5 py-4 text-xs whitespace-nowrap">
                    <span className="px-2.5 py-0.5 bg-white/5 rounded-md border border-white/5 text-foreground font-medium text-[11px]">{e.category}</span>
                  </td>
                  ...
                  <td className="px-5 py-4 font-mono text-right font-bold text-rose-400 whitespace-nowrap text-sm">{fmtINR(e.amount)}</td>
                    ...
                    {isManagerPlus(erpUser) && e.status === "pending" && (
                      <div className="flex gap-1.5 justify-end">
                        <button 
                          ...
                          className="p-1.5 text-emerald-400 hover:bg-emerald-500/10 border border-transparent hover:border-emerald-500/20 rounded-lg transition duration-200 disabled:opacity-40" 
      ...
      >
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <select value={form.category} onChange={e => setForm({...form, category: e.target.value})} className="w-full px-3 py-2 border border-white/10 bg-background rounded-xl text-sm text-foreground focus:outline-none focus:border-accent/40 transition">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input type="number" required value={form.amount} onChange={e => setForm({...form, amount: e.target.value})} placeholder="0.00" className="w-full pl-7 pr-3 py-2 border border-white/10 bg-background/50 rounded-xl text-sm font-mono text-foreground focus:outline-none focus:border-accent/40 transition" data-testid="ce-amount"/>
            </div>
          </div>
          ...
          <div>
            ...
            <input type="text" required value={form.description} onChange={e => setForm({...form, description: e.target.value})} placeholder="Describe operational utility allocation parameters..." className="w-full px-3 py-2 border border-white/10 bg-background/50 rounded-xl text-sm text-foreground focus:outline-none focus:border-accent/40 transition" data-testid="ce-description"/>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="text" value={form.vendor} onChange={e => setForm({...form, vendor: e.target.value})} placeholder="Optional" className="w-full px-3 py-2 border border-white/10 bg-background/50 rounded-xl text-sm text-foreground focus:outline-none focus:border-accent/40 transition" data-testid="ce-vendor"/>
            </div>
            <div>
              <label className="text-xs uppercase tracking-wider font-bold text-muted-foreground mb-1 block">Transaction Date *</label>
              <input type="date" required value={form.expense_date} onChange={e => setForm({...form, expense_date: e.target.value})} className="w-full px-3 py-2 border border-white/10 bg-background/50 rounded-xl text-sm font-mono text-foreground focus:outline-none focus:border-accent/40 transition" data-testid="ce-date"/>
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-3 pt-2">
          ...
          <button type="button" onClick={onClose} className="px-4 py-3 border border-white/10 rounded-xl text-xs uppercase tracking-wider font-bold text-muted-foreground hover:text-foreground hover:bg-white/5 transition">
            Cancel
          </button>
        </div>
      </form>
