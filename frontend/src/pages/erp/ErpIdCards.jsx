    ...
    <div className="space-y-6 h-[calc(100vh-120px)] flex flex-col min-h-0 animate-fadeIn relative">
      ...
      <div className="flex justify-between items-end flex-wrap gap-4 shrink-0 screen-only">
        ...
        <div className="flex items-center gap-3">
          {selectedIds.size > 0 && (
            <button onClick={clearSelection} className="px-3 py-2 border border-white/10 rounded-xl text-xs uppercase tracking-wider font-bold text-muted-foreground hover:text-foreground transition bg-background/30">
      ...
      <div className="flex gap-3 flex-wrap shrink-0 screen-only">
        ...
        <button onClick={selectAllFiltered} className="px-4 py-2 border border-white/10 rounded-xl text-xs font-semibold hover:bg-white/5 transition text-foreground">
          Select All Filtered
        </button>
      </div>

      {/* MAIN SCREEN CONTROL DIRECTORY DIRECT GRID */}
      <div className="glass-elevated rounded-2xl border border-white/5 w-full overflow-hidden flex flex-col flex-1 min-h-0 screen-only">
          ...
          <table className="w-full text-sm table-fixed border-collapse min-w-[750px]">
            <thead className="bg-[#18181b] text-muted-foreground sticky top-0 z-20 shadow-[0_1px_0_rgba(255,255,255,0.05)]">
              <tr className="text-left backdrop-blur-md">
                <th className="w-[6%] px-5 py-3.5 text-center bg-[#131316]"></th>
                <th className="w-[18%] px-5 py-3.5 text-xs font-bold uppercase tracking-wider bg-[#131316]">Student No</th>
                <th className="w-[25%] px-5 py-3.5 text-xs font-bold uppercase tracking-wider bg-[#131316]">Learner Profile Name</th>
                <th className="w-[18%] px-5 py-3.5 text-xs font-bold uppercase tracking-wider bg-[#131316]">Class Batch Allocation</th>
                <th className="w-[15%] px-5 py-3.5 text-xs font-bold uppercase tracking-wider bg-[#131316]">Contact Line</th>
                <th className="w-[18%] px-5 py-3.5 text-xs font-bold uppercase tracking-wider bg-[#131316]">Parent Mobile</th>
              </tr>
            </thead>
              ...
              {filteredStudents.map(s => {
                ...
                return (
                  <tr 
                    ...
                    className={`cursor-pointer transition-colors ${isChecked ? "bg-accent/5 hover:bg-accent/10" : "hover:bg-white/[0.02]"}`}
                  >
