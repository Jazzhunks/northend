          ...
          <table className="w-full text-sm table-fixed border-collapse min-w-[850px]">
            <thead className="bg-[#18181b] text-muted-foreground sticky top-0 z-20 shadow-[0_1px_0_rgba(255,255,255,0.05)]">
              <tr className="text-left backdrop-blur-md">
                <th className="w-[20%] px-5 py-3.5 text-xs font-bold uppercase tracking-wider bg-[#131316]">Staff Identity</th>
                <th className="w-[23%] px-5 py-3.5 text-xs font-bold uppercase tracking-wider bg-[#131316]">Authorization Email</th>
                <th className="w-[15%] px-5 py-3.5 text-xs font-bold uppercase tracking-wider bg-[#131316]">Role Profile</th>
                <th className="w-[17%] px-5 py-3.5 text-xs font-bold uppercase tracking-wider bg-[#131316]">Branch Station</th>
                <th className="w-[15%] px-5 py-3.5 text-xs font-bold uppercase tracking-wider bg-[#131316]">Joined Date</th>
                <th className="w-[10%] px-5 py-3.5 bg-[#131316]"></th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/[0.04] bg-background/20">
              {filteredItems.map(s => (
                <tr key={s.id} className={`hover:bg-white/[0.02] transition-colors group ${s.active === false ? "opacity-35" : ""}`} data-testid={`staff-row-${s.id}`}>
                  ...
                  <td className="px-5 py-4 whitespace-nowrap">
                    <span className={`inline-flex px-2.5 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider font-mono border ${ROLE_STYLES[s.role] || "bg-white/5 border-white/10"}`}>
                      {s.role?.replace("_", " ")}
                    </span>
                  </td>
                  ...
                  <td className="px-5 py-4 text-right whitespace-nowrap pr-6">
                    <div className="flex justify-end gap-2">
                      <button 
                        ...
                        className="p-1.5 text-accent border border-transparent hover:bg-white/5 hover:border-white/10 rounded-lg transition duration-150 disabled:opacity-30"
                      >
                        <Edit3 size={14}/>
                      </button>
                      ...
                      {s.active !== false ? (
                        <button 
                          ...
                          className="p-1.5 text-rose-400 hover:bg-rose-500/10 border border-transparent hover:border-rose-500/20 rounded-lg transition duration-200 disabled:opacity-40" 
...
function CreateStaffModal({ erpUser, branches, onClose, onCreated }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 z-50 grid place-items-center p-4 backdrop-blur-sm animate-fadeIn" onClick={onClose} data-testid="create-staff-modal">
      <form onClick={e => e.stopPropagation()} onSubmit={submit} className="bg-background border border-white/10 rounded-2xl max-w-md w-full p-6 space-y-5 shadow-2xl relative">
        ...
        <div className="space-y-4">
          <div>
            ...
            <input required placeholder="E.g. Junaid Ahmad" value={form.name} onChange={e => setForm({...form, name: e.target.value})} className="w-full px-3 py-2 border border-white/10 bg-background/50 rounded-xl text-sm text-foreground focus:outline-none focus:border-accent/40 transition" data-testid="cs-name"/>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input required type="email" placeholder="username@northendedu.com" value={form.email} onChange={e => setForm({...form, email: e.target.value})} className="w-full pl-9 pr-3 py-2 border border-white/10 bg-background/50 rounded-xl text-sm font-mono text-foreground focus:outline-none focus:border-accent/40 transition" data-testid="cs-email"/>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input type="text" placeholder="Contact string" value={form.phone} onChange={e => setForm({...form, phone: e.target.value})} className="w-full pl-9 pr-3 py-2 border border-white/10 bg-background/50 rounded-xl text-sm font-mono text-foreground focus:outline-none focus:border-accent/40 transition" data-testid="cs-phone"/>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input required type="password" placeholder="••••••••••••" value={form.password} onChange={e => setForm({...form, password: e.target.value})} className="w-full pl-9 pr-3 py-2 border border-white/10 bg-background/50 rounded-xl text-sm font-mono text-foreground focus:outline-none focus:border-accent/40 transition" data-testid="cs-password"/>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select required value={form.role} onChange={e => setForm({...form, role: e.target.value})} className="w-full px-3 py-2 border border-white/10 bg-background rounded-xl text-sm text-foreground focus:outline-none focus:border-accent/40 transition" data-testid="cs-role">
            ...
            <div>
              ...
              <select required value={form.branch_id} onChange={e => setForm({...form, branch_id: e.target.value})} disabled={!isSuper(erpUser)} className="w-full px-3 py-2 border border-white/10 bg-background rounded-xl text-sm text-foreground focus:outline-none focus:border-accent/40 transition disabled:opacity-50" data-testid="cs-branch">
...
function UpdateStaffModal({ erpUser, branches, staffMember, onClose, onUpdated }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 z-50 grid place-items-center p-4 backdrop-blur-sm animate-fadeIn" onClick={onClose}>
      <form onClick={e => e.stopPropagation()} onSubmit={submitUpdate} className="bg-background border border-white/10 rounded-2xl max-w-md w-full p-6 space-y-5 shadow-2xl relative">
        <div className="flex justify-between items-start">
          <div>
            <div className="text-xs uppercase tracking-[0.2em] font-bold text-accent flex items-center gap-1">
        ...
        <div className="space-y-4">
          <div>
            <label className="text-xs uppercase tracking-wider font-bold text-muted-foreground mb-1 block">Full Name *</label>
            <input required placeholder="E.g. Junaid Ahmad" value={form.name} onChange={e => setForm({...form, name: e.target.value})} className="w-full px-3 py-2 border border-white/10 bg-background/50 rounded-xl text-sm text-foreground focus:outline-none focus:border-accent/40 transition" />
          </div>
          ...
          <div>
            ...
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground/40"><Smartphone size={14}/></span>
              <input type="text" placeholder="Contact string" value={form.phone} onChange={e => setForm({...form, phone: e.target.value})} className="w-full pl-9 pr-3 py-2 border border-white/10 bg-background/50 rounded-xl text-sm font-mono text-foreground focus:outline-none focus:border-accent/40 transition" />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground/40"><KeyRound size={14}/></span>
              <input type="password" placeholder="Leave empty to retain current password" value={form.new_password} onChange={e => setForm({...form, new_password: e.target.value})} className="w-full pl-9 pr-3 py-2 border border-white/10 bg-background/50 rounded-xl text-sm font-mono text-foreground focus:outline-none focus:border-accent/40 transition" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs uppercase tracking-wider font-bold text-muted-foreground mb-1 block">Assigned Role *</label>
              <select required value={form.role} onChange={e => setForm({...form, role: e.target.value})} className="w-full px-3 py-2 border border-white/10 bg-background rounded-xl text-sm text-foreground focus:outline-none focus:border-accent/40 transition">
                {allowedRoles.map(r => <option key={r} value={r}>{r.replace("_", " ").toUpperCase()}</option>)}
              </select>
            </div>
            ...
            <div>
              <label className="text-xs uppercase tracking-wider font-bold text-muted-foreground mb-1 block">Station Branch *</label>
              <select required value={form.branch_id} onChange={e => setForm({...form, branch_id: e.target.value})} disabled={!isSuper(erpUser)} className="w-full px-3 py-2 border border-white/10 bg-background rounded-xl text-sm text-foreground focus:outline-none focus:border-accent/40 transition disabled:opacity-50">
                {branches.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}
              </select>
            </div>
          </div>
