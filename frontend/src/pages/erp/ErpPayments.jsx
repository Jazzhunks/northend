      ...
      <div className="flex justify-between items-end flex-wrap gap-4 shrink-0">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            {filteredItems.length} receipts compiled • Gross collection total: <span className="text-emerald-600 font-semibold font-mono">{fmtINR(total)}</span>
          </p>
        </div>
        <a href={`${API_BASE}/erp/exports/payments.xlsx`} target="_blank" rel="noreferrer" className="block">
          <button className="px-4 py-2.5 border border-border rounded-xl text-xs uppercase tracking-wider font-bold text-muted-foreground hover:text-foreground hover:bg-muted/50 flex items-center gap-2 transition" data-testid="export-payments-btn">
      ...
      <div className="flex gap-3 flex-wrap shrink-0 items-center">
        ...
        {isSuper(erpUser) && (
          <select 
            ...
            className="border border-border rounded-xl px-4 py-2 bg-background/50 text-sm min-w-[180px] focus:outline-none text-foreground" 
        ...
        <div className="flex items-center gap-2 border border-border bg-background/30 rounded-xl px-3 py-1">
          ...
          <table className="w-full text-sm table-fixed border-collapse min-w-[850px]">
            <thead className="bg-muted text-muted-foreground sticky top-0 z-20 shadow-[0_1px_0_rgba(255,255,255,0.05)]">
              <tr className="text-left backdrop-blur-md">
                <th className="w-[15%] px-5 py-3.5 text-xs font-bold uppercase tracking-wider bg-muted">Receipt ID</th>
                <th className="w-[15%] px-5 py-3.5 text-xs font-bold uppercase tracking-wider bg-muted">Collection Date</th>
                <th className="w-[18%] px-5 py-3.5 text-xs font-bold uppercase tracking-wider bg-muted">Student No</th>
                <th className="w-[12%] px-5 py-3.5 text-xs font-bold uppercase tracking-wider bg-muted">Method Mode</th>
                <th className="w-[17%] px-5 py-3.5 text-xs font-bold uppercase tracking-wider bg-muted">Collected By</th>
                <th className="w-[15%] px-5 py-3.5 text-xs font-bold uppercase tracking-wider text-right bg-muted">Net Amount</th>
                <th className="w-[8%] px-5 py-3.5 bg-muted"></th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border bg-background/20">
              {filteredItems.map(p => (
                <tr key={p.id} className="hover:bg-muted/50 transition-colors group" data-testid={`pay-row-${p.id}`}>
                  ...
                  <td className="px-5 py-4 font-mono text-right font-bold text-emerald-600 whitespace-nowrap text-sm">{fmtINR(p.amount)}</td>
...
function ModeBadge({ mode }) {
  ...
  let config = "bg-muted/50 text-muted-foreground border-border"; // Fallback details
