  ...
  return (
    <div className="space-y-6 h-[calc(100vh-120px)] flex flex-col min-h-0 animate-fadeIn relative" data-testid="erp-attendance-page">
      ...
      <div className="flex justify-between items-end flex-wrap gap-4 shrink-0">
        ...
        <div className="flex items-center gap-3">
          <a href={buildExcelExportUrl()} target="_blank" rel="noreferrer" className="block">
            <button 
              disabled={!branchId}
              className="px-4 py-2.5 bg-zinc-900 border border-white/10 text-foreground font-bold text-xs uppercase tracking-wider rounded-xl hover:bg-zinc-800 disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-2 transition"
            >
              <FileDown size={14} className="text-emerald-400" /> Export Today's Excel
            </button>
          </a>
      ...
      <div className="grid md:grid-cols-3 gap-4 shrink-0">
        <div className="glass-elevated p-4 rounded-xl border border-white/5 bg-background/40 md:col-span-2 flex flex-col justify-center">
          ...
          <form onSubmit={handleTerminalScanSubmit} className="flex gap-2">
            <input 
              ...
              className="flex-1 px-3 py-2 border border-white/10 bg-background/80 font-mono text-sm uppercase rounded-xl text-foreground focus:outline-none focus:border-accent transition disabled:opacity-50"
            />
            <button 
              type="submit"
        ...
        <div className="glass-elevated p-4 rounded-xl border border-white/5 bg-background/40 flex flex-col justify-center">
          <label className="text-xs uppercase font-bold tracking-wider text-muted-foreground mb-1.5 flex items-center gap-1">
          ...
          {isSuper(erpUser) ? (
            <select
              ...
              className="w-full border border-white/10 rounded-xl px-3 py-2 bg-background/80 text-sm text-foreground focus:outline-none focus:border-accent transition"
            >
              <option value="">— Select Monitoring Station —</option>
              {branches.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}
            </select>
          ) : (
            <div className="px-3 py-2 border border-white/5 bg-white/[0.02] text-sm text-foreground rounded-xl font-medium truncate">
      ...
      <div className="grid lg:grid-cols-3 gap-6 flex-1 min-h-0">
        <div className="glass-elevated rounded-2xl border border-white/5 overflow-hidden flex flex-col lg:col-span-2 min-h-0 bg-background/10">
          <div className="p-4 border-b border-white/[0.04] bg-background/40 flex justify-between items-center shrink-0">
            ...
            <input 
              ...
              className="px-3 py-1 border border-white/10 bg-background/50 rounded-lg text-xs text-foreground focus:outline-none focus:border-accent/40 w-48 transition"
            />
          </div>
          ...
          <div className="overflow-y-auto overflow-x-auto w-full h-full custom-scrollbar min-h-0">
            <table className="w-full text-sm table-fixed border-collapse min-w-[550px]">
              <thead className="bg-[#18181b]/90 backdrop-blur-md text-muted-foreground sticky top-0 z-20 shadow-[0_1px_0_rgba(255,255,255,0.05)]">
                <tr className="text-left">
                  <th className="w-[20%] px-4 py-3 text-xs font-bold uppercase tracking-wider bg-[#131316]">Reg Code</th>
                  <th className="w-[35%] px-4 py-3 text-xs font-bold uppercase tracking-wider bg-[#131316]">Student Full Name</th>
                  <th className="w-[25%] px-4 py-3 text-xs font-bold uppercase tracking-wider bg-[#131316]">Verified Clock</th>
                  <th className="w-[20%] px-4 py-3 text-xs font-bold uppercase tracking-wider text-center bg-[#131316]">Status Block</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/[0.04]">
                {filteredLogs.map(log => (
                  <tr key={log.id} className="hover:bg-white/[0.01] transition-colors bg-background/10 animate-slideUp">
        ...
        <div className="glass-elevated rounded-2xl border border-white/5 overflow-hidden flex flex-col min-h-0 bg-background/10">
          ...
          <div className="overflow-y-auto p-4 space-y-2.5 flex-1 min-h-0 custom-scrollbar">
            {isManagerPlus(erpUser) ? (
              students
                ...
                .map(st => (
                  <div key={st.id} className="p-3 border border-white/5 bg-background/40 rounded-xl flex items-center justify-between gap-3 group hover:border-white/10 transition">
                    ...
                    <div className="flex gap-1 shrink-0">
                      <button
                        ...
                        className="px-2 py-1 bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/20 text-emerald-400 rounded-md text-[10px] font-bold uppercase tracking-wider transition disabled:opacity-30"
                      >
                        Present
                      </button>
                      <button
