...
const Stat = ({ label, value, icon: Icon, accent, testid }) => (
  <div className="glass-elevated rounded-2xl p-6 relative overflow-hidden group border border-white/5" data-testid={testid}>
...
const CustomTooltip = ({ active, payload, label }) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-background/95 border border-white/10 p-4 rounded-xl shadow-xl backdrop-blur-md z-50">
    ...
    <div className="p-1 sm:p-6 space-y-8 relative">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <h1 className="font-display text-4xl lg:text-5xl font-light tracking-tight mt-2">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button onClick={() => setActiveModal("duelist")} className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 text-xs font-bold uppercase tracking-wider transition border border-amber-500/20 shadow-md">
            <Clock size={14} /> Today's Dues
          </button>
          {erpUser.role !== "counsellor" && (
            <button onClick={() => setActiveModal("admission")} className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary text-primary-foreground text-xs font-bold uppercase tracking-wider hover:bg-primary/90 transition shadow-lg">
              <Plus size={14} /> New Admission
            </button>
          )}
          {canSeeFinance && (
            <button onClick={() => setActiveModal("expense")} className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 hover:bg-white/20 text-foreground text-xs font-bold uppercase tracking-wider transition">
              <Wallet size={14} /> Add Expense
            </button>
          )}
          <button onClick={() => setActiveModal("students")} className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 hover:bg-white/10 text-muted-foreground hover:text-foreground text-xs font-bold uppercase tracking-wider transition border border-white/5">
            <Users size={14} /> Records
          </button>
          {canSeeFinance && (
            <button onClick={() => setActiveModal("cashbook")} className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 hover:bg-white/10 text-muted-foreground hover:text-foreground text-xs font-bold uppercase tracking-wider transition border border-white/5">
              <IndianRupee size={14} /> Cashbook
            </button>
          )}
          {canSeeFinance && (
            <button onClick={() => setActiveModal("outflow")} className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 hover:bg-white/10 text-muted-foreground hover:text-foreground text-xs font-bold uppercase tracking-wider transition border border-white/5">
              <Layers size={14} /> Outflow
            </button>
          )}
        </div>
      </div>
...
function TodayDueListModal({ erpUser, onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 z-50 flex justify-end backdrop-blur-sm animate-fadeIn" onClick={onClose}>
      <div onClick={e => e.stopPropagation()} className="bg-background border-l border-white/10 w-full max-w-2xl h-full p-6 flex flex-col justify-between overflow-y-auto shadow-2xl">
        <div className="space-y-6 flex-1 flex flex-col min-h-0">
          <div className="flex justify-between items-start shrink-0">
            ...
            <button onClick={onClose} className="p-2 border border-white/10 rounded-xl hover:bg-white/5 transition"><X size={16}/></button>
          </div>
          ...
          <div className="relative w-full shrink-0">
            ...
            <input 
              ...
              className="w-full pl-9 pr-4 py-2 border border-white/10 bg-background/50 rounded-xl text-sm focus:outline-none focus:border-amber-500/40 transition font-sans text-foreground"
            />
          </div>
          ...
          {loading ? (
            ...
            <div className="glass-elevated rounded-2xl border border-white/5 flex-1 overflow-y-auto overflow-x-hidden min-h-0">
              <table className="w-full text-sm">
                <thead className="bg-white/[0.03] text-muted-foreground sticky top-0 backdrop-blur-md z-10 border-b border-white/5">
                  ...
                  {runtimeFilteredDues.map(s => (
                    <tr key={s.id} className="hover:bg-white/[0.01] transition-colors group">
                      <td className="px-5 py-4">
                        ...
                        <div className="text-[11px] font-mono text-muted-foreground mt-1 flex flex-wrap items-center gap-x-2 divide-x divide-white/10">
                          <span>{s.student_no}</span>
                          <span className="pl-2">Batch: {s.batch || "Unallocated"}</span>
                        </div>
                      </td>
                      <td className="px-5 py-4 font-mono text-right font-bold text-sm text-rose-400">
                      ...
                      <td className="px-5 py-4 text-right">
                        <button 
                          ...
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 text-xs font-bold transition duration-300 border border-emerald-500/10 shadow-sm"
                        >
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <Stat label="Total Revenue" value={fmtINR(d.total_revenue)} icon={TrendingUp} accent="text-emerald-400" testid="stat-revenue"/>
        <Stat label="Total Expense" value={fmtINR(d.total_expense)} icon={TrendingDown} accent="text-rose-400" testid="stat-expense"/>
        <Stat label="Net Income" value={fmtINR(d.net_income)} icon={Wallet} accent={d.net_income >= 0 ? "text-emerald-400" : "text-rose-400"} testid="stat-net"/>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="lg:col-span-2 glass-elevated rounded-2xl p-6 border border-white/5">
      ...
      <div className="glass-elevated rounded-2xl overflow-hidden border border-white/5" data-testid="branches-table">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-white/[0.06]">
              {(d.branches || []).map(b => (
                <tr key={b.branch_id} className="hover:bg-white/[0.02] transition-colors" data-testid={`branch-row-${b.branch_id}`}>
                  ...
                  <td className="px-6 py-4 font-mono text-right text-emerald-400/90">{fmtINR(b.revenue)}</td>
                  <td className="px-6 py-4 font-mono text-right text-rose-400/90">{fmtINR(b.expense)}</td>
                  <td className="px-6 py-4 text-right">
                    <span className={`inline-flex px-2.5 py-1 rounded-md font-mono font-bold text-xs ${b.net >= 0 ? "bg-emerald-400/10 text-emerald-400" : "bg-rose-400/10 text-rose-400"}`}>
                      {fmtINR(b.net)}
                    </span>
                  </td>
                </tr>
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {canSeeFinance && (
          <>
            <Stat label="Gross Revenue" value={fmtINR(d.revenue)} icon={TrendingUp} accent="text-emerald-400" testid="stat-revenue"/>
            <Stat label="Total Expense" value={fmtINR(d.expense)} icon={TrendingDown} accent="text-rose-400" testid="stat-expense"/>
          </>
        )}
      ...
      <div className={`grid grid-cols-1 ${canSeeFinance ? "lg:grid-cols-2" : "lg:grid-cols-1"} gap-5`}>
        {canSeeFinance && (
          <div className="glass-elevated rounded-2xl p-6 flex flex-col border border-white/5" data-testid="expense-breakdown">
        ...
        <div className="glass-elevated rounded-2xl p-6 flex flex-col border border-white/5" data-testid="counsellor-performance">
      ...
      {d.recent_payments?.length > 0 && (
        <div className="glass-elevated rounded-2xl overflow-hidden border border-white/5 animate-fadeIn" data-testid="recent-payments">
          <div className="px-6 py-5 border-b border-white/[0.06] flex items-center justify-between">
              ...
              <tbody className="divide-y divide-white/[0.06]">
                {d.recent_payments.map(p => (
                  <tr key={p.id} className="hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className="inline-flex px-2 py-1 rounded bg-white/5 text-[10px] uppercase tracking-wider font-bold text-muted-foreground">
                        {p.mode}
                      </span>
                    </td>
                    <td className="px-6 py-4 font-mono text-right text-emerald-400 font-medium">{fmtINR(p.amount)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function CashbookViewModal({ erpUser, onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 z-50 flex justify-end backdrop-blur-sm animate-fadeIn" onClick={onClose}>
      <div onClick={e => e.stopPropagation()} className="bg-background border-l border-white/10 w-full max-w-4xl h-full p-6 flex flex-col justify-between overflow-y-auto shadow-2xl">
        <div className="space-y-6">
          <div className="flex justify-between items-start">
            ...
            <div className="flex items-center gap-2">
              <a href={`${API_BASE}/erp/exports/payments.xlsx`} target="_blank" rel="noreferrer" className="p-2 border border-white/10 rounded-xl hover:bg-white/5 transition">
                <Download size={16}/>
              </a>
              <button onClick={onClose} className="p-2 border border-white/10 rounded-xl hover:bg-white/5 transition"><X size={16}/></button>
            </div>
          ...
          <div className="flex gap-3 flex-wrap">
            {isSuper(erpUser) && (
              <select value={branchId} onChange={e => setBranchId(e.target.value)} className="border border-white/10 rounded-xl px-3 py-2 bg-background/50 text-sm flex-1 min-w-[150px]">
                <option value="">All branches</option>
                {branches.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}
              </select>
            )}
            <input type="date" value={from} onChange={e => setFrom(e.target.value)} className="border border-white/10 rounded-xl px-3 py-2 bg-background/50 text-sm flex-1"/>
            <input type="date" value={to} onChange={e => setTo(e.target.value)} className="border border-white/10 rounded-xl px-3 py-2 bg-background/50 text-sm flex-1"/>
          </div>
                ...
                <tbody className="divide-y divide-white/[0.06]">
                  {items.map(p => (
                    <tr key={p.id} className="hover:bg-white/[0.01] transition-colors">
                      ...
                      <td className="px-4 py-3 text-xs uppercase"><span className="px-1.5 py-0.5 bg-white/5 rounded text-[10px] font-bold">{p.mode}</span></td>
                      <td className="px-4 py-3 text-xs text-muted-foreground">{p.collected_by_name || "—"}</td>
                      <td className="px-4 py-3 font-mono text-right font-bold text-emerald-400">{fmtINR(p.amount)}</td>
...
function StudentsViewModal({ erpUser, onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 z-50 flex justify-end backdrop-blur-sm animate-fadeIn" onClick={onClose}>
      <div onClick={e => e.stopPropagation()} className="bg-background border-l border-white/10 w-full max-w-5xl h-full p-6 flex flex-col gap-6 overflow-y-auto shadow-2xl">
        <div className="flex justify-between items-start">
          <div>
          ...
          <div className="flex items-center gap-2">
            <a href={`${API_BASE}/erp/exports/students.xlsx`} target="_blank" rel="noreferrer" className="p-2 border border-white/10 rounded-xl hover:bg-white/5 transition">
              <Download size={16}/>
            </a>
            <button onClick={onClose} className="p-2 border border-white/10 rounded-xl hover:bg-white/5 transition"><X size={16}/></button>
          </div>
        ...
        <div className="flex gap-3 flex-wrap">
          <div className="relative flex-1 min-w-[250px]">
            <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground"/>
            <input value={q} onChange={e => setQ(setQ.target.value)} placeholder="Search by full name, registration number, phone..." className="w-full pl-9 pr-4 py-2 border border-white/10 bg-background/50 rounded-xl text-sm"/>
          </div>
          {isSuper(erpUser) && (
            <select value={branchId} onChange={e => setBranchId(e.target.value)} className="border border-white/10 rounded-xl px-3 py-2 bg-background/50 text-sm min-w-[180px]">
  ...
  return (
    <div className="fixed inset-0 bg-black/80 z-50 flex justify-end backdrop-blur-sm animate-fadeIn" onClick={onClose}>
      <div 
        ...
        className="bg-background border-l border-white/10 w-full max-w-5xl h-full p-6 flex flex-col justify-between shadow-2xl overflow-hidden"
      >
        <div className="space-y-6 flex flex-col h-full min-h-0">
          ...
          <div className="flex justify-between items-start shrink-0">
            <div>
              ...
              <p className="text-muted-foreground text-sm mt-1">
                {items.length} entries registered • Total approved execution: <span className="text-rose-400 font-bold">{fmtINR(total)}</span>
              </p>
            </div>
            <div className="flex items-center gap-2">
              <a href={`${API_BASE}/erp/exports/expenses.xlsx`} target="_blank" rel="noreferrer" className="p-2 border border-white/10 rounded-xl hover:bg-white/5 transition">
                <Download size={16}/>
              </a>
              <button onClick={onClose} className="p-2 border border-white/10 rounded-xl hover:bg-white/5 transition"><X size={16}/></button>
            </div>
          ...
          <div className="flex gap-3 flex-wrap shrink-0">
            {isSuper(erpUser) && (
              <select value={branchId} onChange={e => setBranchId(e.target.value)} className="border border-white/10 rounded-xl px-3 py-2 bg-background/50 text-sm flex-1 min-w-[150px] focus:outline-none text-foreground">
                <option value="">All branches</option>
                {branches.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}
              </select>
            )}
            <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="border border-white/10 rounded-xl px-3 py-2 bg-background/50 text-sm flex-1 min-w-[150px] focus:outline-none text-foreground">
          ...
          <div className="glass-elevated rounded-2xl border border-white/5 w-full overflow-hidden flex flex-col h-[calc(100vh-240px)] min-h-0">
              ...
              <table className="w-full text-sm table-auto border-collapse">
                <thead className="bg-[#18181b] text-muted-foreground sticky top-0 z-20 shadow-[0_1px_0_rgba(255,255,255,0.05)]">
                  <tr className="text-left backdrop-blur-md">
                    <th className="px-4 py-3.5 text-xs font-bold uppercase tracking-wider bg-[#131316]">Date</th>
                    <th className="px-4 py-3.5 text-xs font-bold uppercase tracking-wider bg-[#131316]">Category</th>
                    <th className="px-4 py-3.5 text-xs font-bold uppercase tracking-wider bg-[#131316]">Description</th>
                    <th className="px-4 py-3.5 text-xs font-bold uppercase tracking-wider bg-[#131316]">Vendor/Party</th>
                    <th className="px-4 py-3.5 text-xs font-bold uppercase tracking-wider bg-[#131316]">Status</th>
                    <th className="px-4 py-3.5 text-xs font-bold uppercase tracking-wider text-right bg-[#131316]">Amount</th>
                    <th className="px-4 py-3.5 bg-[#131316]"></th>
                  </tr>
                </thead>
                  ...
                  {items.map(e => (
                    <tr key={e.id} className="hover:bg-white/[0.02] transition-colors group">
                      ...
                      <td className="px-4 py-3.5 text-xs whitespace-nowrap">
                        <span className="px-2 py-0.5 bg-white/5 rounded-md border border-white/5 text-foreground font-medium">{e.category}</span>
                      </td>
                      ...
                      <td className="px-4 py-3.5 font-mono text-right font-bold text-rose-400 whitespace-nowrap">{fmtINR(e.amount)}</td>
                      <td className="px-4 py-3.5 text-right whitespace-nowrap">
                        {isManagerPlus(erpUser) && e.status === "pending" && (
                          <div className="flex gap-1.5 justify-end">
                            <button onClick={() => handleDecision(e.id, "approve")} className="p-1 text-emerald-400 hover:bg-emerald-500/10 border border-emerald-500/0 hover:border-emerald-500/20 rounded-lg transition duration-200"><Check size={14}/></button>
                            <button onClick={() => handleDecision(e.id, "reject")} className="p-1 text-rose-400 hover:bg-rose-500/10 border border-rose-500/0 hover:border-rose-500/20 rounded-lg transition duration-200"><Ban size={14}/></button>
                          </div>
                        )}
                      </td>
                    </tr>
...
function CreateStudentModal({ erpUser, onClose, onCreated }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 z-50 grid place-items-center p-4 backdrop-blur-sm overflow-y-auto animate-fadeIn" onClick={onClose}>
      <form onClick={e => e.stopPropagation()} onSubmit={executeSubmit} className="bg-background border border-white/10 rounded-2xl max-w-2xl w-full p-6 space-y-5 my-8 shadow-2xl">
        <div className="flex justify-between items-start">
          <div>
          ...
          <button type="button" onClick={onClose} className="p-1 hover:bg-white/5 rounded-lg"><X size={18}/></button>
        </div>
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <div className="sm:col-span-2">
            ...
            <textarea value={form.address} onChange={e => setForm({...form, address: e.target.value})} rows={2} className="w-full px-3 py-2 border border-white/10 bg-background/50 rounded-xl text-sm focus:outline-none focus:border-accent" />
          </div>
        </div>
...
function CreateExpenseModal({ erpUser, onClose, onCreated }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 z-50 grid place-items-center p-4 backdrop-blur-sm animate-fadeIn" onClick={onClose}>
      <form onClick={e => e.stopPropagation()} onSubmit={executeSubmit} className="bg-background border border-white/10 rounded-2xl max-w-md w-full p-6 space-y-4 shadow-2xl">
        <div className="flex justify-between items-start">
          <div>
        ...
        {isSuper(erpUser) && (
          <div>
            ...
            <select required value={form.branch_id} onChange={e => setForm({...form, branch_id: e.target.value})} className="w-full px-3 py-2 border border-white/10 bg-background rounded-xl text-sm focus:outline-none">
        ...
        <div>
          ...
          <select value={form.category} onChange={e => setForm({...form, category: e.target.value})} className="w-full px-3 py-2 border border-white/10 bg-background rounded-xl text-sm focus:outline-none">
...
function InputCard({ label, v, on, type = "text", req, placeholder }) {
  return (
    <div>
      ...
      <input type={type} value={v} required={req} placeholder={placeholder} onChange={e => on(e.target.value)} className="w-full px-3 py-2 border border-white/10 bg-background/50 rounded-xl text-sm focus:outline-none focus:border-accent font-mono text-foreground placeholder:text-muted-foreground/50" />
    </div>
  );
}
...
function SelectCard({ label, v, on, opts, req, disabled }) {
  return (
    <div>
      <label className="text-xs uppercase tracking-wider font-bold text-muted-foreground mb-1 block">{label}{req && " *"}</label>
      <select value={v} onChange={e => on(e.target.value)} required={req} disabled={disabled} className="w-full px-3 py-2 border border-white/10 bg-background/50 rounded-xl text-sm focus:outline-none focus:border-accent disabled:opacity-50">
