    ...
    <div className="relative min-h-[calc(100vh-64px)]" data-testid="student-dashboard">
      ...
      <div className="relative max-w-7xl mx-auto px-4 lg:px-8 pt-12 lg:pt-20 pb-24">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}>
          ...
          <h1 className="font-display text-5xl lg:text-7xl font-light tracking-tight mt-4 leading-[0.95]">
                ...
                <div className="divide-y divide-white/[0.06]">
                  {enrollments.map(e => (
                    <div key={e.id} className="p-5 flex items-center justify-between" data-testid={`enr-row-${e.id}`}>
                      ...
                      <span className={`text-[10px] uppercase tracking-[0.22em] font-bold px-3 py-1 rounded-full ${e.status === "approved" ? "bg-emerald-500/15 text-emerald-400" : "bg-amber-500/15 text-amber-400"}`}>{e.status}</span>
                    </div>
                  ))}
                </div>
              )}
