...
export default function InquiriesTab({ innerSearch }) {
  const [page, setPage] = useState(1);
  const limit = 25;
  const skip = (page - 1) * limit;

  const { data, isLoading } = useQuery({
    queryKey: ['inquiries', { page, innerSearch }],
    queryFn: async () => {
  ...
  const dl = async () => {
    setExporting(true);
    try {
      const res = await api.get(`/admin/export/inquiries`, { responseType: "blob" });
      const blob = new Blob([res.data]);
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url; a.download = `inquiries.xlsx`; a.click(); URL.revokeObjectURL(url);
              ...
              <tbody className="divide-y divide-border text-foreground bg-background/20">
                {inquiries.map((i) => (
                  <tr key={i.id} className="hover:bg-muted/50 transition-colors">
                    ...
                    <td className="p-3 sm:p-4 text-foreground font-medium truncate max-w-[200px]" title={i.subject}>{i.subject}</td>
