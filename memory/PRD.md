...
## Recent History
- **Jun 2026** — **Carnival slot urgency (display-only gimmick)**: public `/wath/page` & `/wath/carnivals/{id}` now show slots ~50-70% full (deterministic per-slot padding in `_hydrate_carnival(pad=True)`) to drive urgency, while `available` and real capacity stay truthful so genuine registrations are never blocked. Admin views use `pad=False` (real numbers). Verified via curl (public padded vs admin real).
