...
## Recent History
- **Jun 2026** — **Admit card redesigned** to match northendedu.com brand: `pdf_client.admit_card_pdf` now uses a blue→green gradient header (site `--primary #1483D2` → `--accent #08BA7F`), glass-style rounded candidate/exam cards, blue & green soft accent pills (target track / date-time / venue), QR "scan for result", green-bullet instructions, and a branded footer. Verified via rendered PDF.
- **Jun 2026** — **Carnival count fix**: closed/full slots were still adding their padded "remaining" to the date-tab total (showed 47 vs real 33). `_hydrate_carnival` now forces closed/full slots to `remaining=0` / `booked=capacity` in the PUBLIC padded view only (admin keeps real numbers), so date totals equal the sum of available slots. Verified via curl (33) + UI.
